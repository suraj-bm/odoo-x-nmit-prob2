...
class Command(BaseCommand):
    help = 'Seed the database with simple sample data'

    def handle(self, *args, **options):
        self.stdout.write('Starting to seed simple data...')
        ...
        with transaction.atomic():
            ...
            # Create simple transactions without signals
            self.create_simple_transactions()
        
        self.stdout.write(
            self.style.SUCCESS('Successfully seeded simple data!')
        )
    ...
    def create_simple_transactions(self):
        """Create simple transactions without triggering signals"""
        vendors = list(Contact.objects.filter(contact_type='vendor'))
        ...
        for i in range(5):
            ...
            so_date = date.today() - timedelta(days=random.randint(1, 20))
            expected_delivery = so_date + timedelta(days=random.randint(3, 14))
        ...
        for i in range(4):
            ...
            invoice = CustomerInvoice.objects.create(
                ...
                created_by=admin_user,
                subtotal=Decimal('1000.00') + Decimal(str(random.randint(100, 2000))),
                tax_amount=Decimal('180.00') + Decimal(str(random.randint(18, 360))),
                total_amount=Decimal('1180.00') + Decimal(str(random.randint(118, 2360)))
            )
            
            invoice_count += 1
        
        self.stdout.write(f'Created {po_count} purchase orders, {so_count} sales orders, and {invoice_count} invoices')
